...
public class ImageController: Controller
{
    ...
    public async Task<IActionResult> Collections(string type, string path, [FromQuery] ImageConvertArguments request)
    {
        try
        {
            ...
            {
                await ImageClient.Download("/" + path, "original", true);
            }
            ...
            string originalMimeType = MimeMapping.MimeUtility.GetMimeMapping(filePath);
            
            bool emptyArguments = request.Width is null && request.Type is null && request.Quality is 100;

            if (emptyArguments || path.Contains(".svg") || (originalFileSize < request.Width && originalMimeType == request.Type.ToString()))
            {
            ...
            {
                return PhysicalFile(cachedImagePath, "image/" + request.Type);
            }
