...
public record PersonResponseItemDto
{
    ...
    {
        var biography = tmdbPersonAppends.Translations.Translations
        ...
        var cast = tmdbPersonAppends.CombinedCredits.Cast
            .Select(cast => new KnownFor(cast, person))
            .DistinctBy(knownFor => knownFor.Id)
            .ToArray();

        var crew = tmdbPersonAppends.CombinedCredits.Crew
            .Select(crew => new KnownFor(crew, person))
            .DistinctBy(knownFor => knownFor.Id)
            .ToArray();
