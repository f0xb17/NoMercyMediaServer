...
public record InfoResponseItemDto
{
    ...
    {
        var title = tmdbMovie.Translations.Translations
            .FirstOrDefault(translation => translation.Iso31661 == country)?
            .Data.Title;

        var overview = tmdbMovie.Translations.Translations
            .FirstOrDefault(translation => translation.Iso31661 == country)?
            .Data.Overview;
        ...
        var crew = tmdbMovie.Credits.Crew
            .Select(crew => new PeopleDto(crew))
            .ToArray();
    ...
    public InfoResponseItemDto(Tv tv, string? country)
    {
        ...
        var similarIds = tv.SimilarFrom
            .Select(similar => similar.MediaId);
        var similars = mediaContext.Tvs
        ...
        var recommendationIds = tv.RecommendationFrom
            .Select(recommendation => recommendation.MediaId);
        var recommendations = mediaContext.Tvs
    ...
    {
        var title = tmdbTv.Translations.Translations
            .FirstOrDefault(translation => translation.Iso31661 == country)?
            .Data.Title;

        var overview = tmdbTv.Translations.Translations
            .FirstOrDefault(translation => translation.Iso31661 == country)?
            .Data.Overview;
        ...
        var cast = tmdbTv.Credits.Cast
            .Select(cast => new PeopleDto(cast))
            .ToArray();

        var crew = tmdbTv.Credits.Crew
            .Select(crew => new PeopleDto(crew))
            .ToArray();
    ...
    public InfoResponseItemDto(Collection collection, string country)
    {
        ...
        var cast = collection.CollectionMovies
            .SelectMany(collectionMovie => collectionMovie.Movie.Cast)
            .Select(cast => new PeopleDto(cast))
            .ToArray();

        var crew = collection.CollectionMovies
...
public record SeasonDto
{
    ...
    {
        var title = season.Translations.FirstOrDefault()?.Title;
        var overview = season.Translations.FirstOrDefault()?.Overview;
    ...
    public SeasonDto(int tvId, Providers.TMDB.Models.Season.TmdbSeason tmdbSeason, string country)
    {
        TmdbSeasonClient tmdbSeasonClient = new(tvId, tmdbSeason.SeasonNumber);
        var seasonData = tmdbSeasonClient.WithAllAppends().Result;

        var title = seasonData?.Translations.Translations
            .FirstOrDefault(translation => translation.Iso31661 == country)?
            .Data.Title;

        var overview = seasonData?.Translations.Translations
            .FirstOrDefault(translation => translation.Iso31661 == country)?
            .Data.Overview;
...
public record EpisodeDto
{
    ...
    {
        var title = episode.Translations.FirstOrDefault()?.Title;
        var overview = episode.Translations.FirstOrDefault()?.Overview;
    ...
    public EpisodeDto(int tvId, int seasonNumber, int episodeNumber, string country)
    {
        ...
        var episodeData = tmdbEpisodeClient.WithAllAppends().Result;

        if (episodeData is null) return;

        var title = episodeData.Translations.Translations
            .FirstOrDefault(translation => translation.Iso31661 == country)?
            .Data.Title;

        var overview = episodeData.Translations.Translations
            .FirstOrDefault(translation => translation.Iso31661 == country)?
            .Data.Overview;
