...
public record LibrariesResponseItemDto
{
    ...
    {
        var image = library.Type switch
        {
            "anime" => library.LibraryTvs?.Randomize()?.FirstOrDefault(lt => lt.Library.Type == "anime")?.Tv.Backdrop ?? library.Image ?? "",
            "movie" => library.LibraryMovies?.Randomize()?.FirstOrDefault(lm => lm.Library.Type == "movie")?.Movie.Backdrop ?? library.Image ?? "",
            "tv" => library.LibraryTvs?.Randomize()?.FirstOrDefault(lt => lt.Library.Type == "tv")?.Tv.Backdrop ?? library.Image ?? "",
            _ => library.Image ?? ""
        };
