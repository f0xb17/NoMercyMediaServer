...
public class TvClient : BaseClient
{
    ...
    public Task<TvShowAppends?> WithAppends(string[] appendices, bool? priority = false)
    {
        ...
        return Get<TvShowAppends>("tv/" + Id, queryParams, priority);
    }

    public Task<TvShowAppends?> WithAllAppends(bool? priority = false)
    {
        return WithAppends([
            "aggregate_credits",
            "alternative_titles",
    ...
    public Task<TvChanges?> Changes(string startDate, string endDate)
    {
        ...
        return Get<TvChanges>("tv/" + Id + "/changes", queryParams);
    }
