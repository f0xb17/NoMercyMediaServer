...
{
    private readonly MediaContext _context = new();
    ...
    private async Task Store()
    {
        Collection collections = new()
        {
            ...
            TitleSort = collectionAppends.Name.TitleSort(collectionAppends.Parts.MinBy(movie => movie.ReleaseDate)
                ?.ReleaseDate),
        ...
        await _context.Collections.Upsert(collections)
            .On(v => new { v.Id })
        ...
        await _context.Movies.UpsertRange(movies)
            .On(v => new { v.Id })
        ...
        CollectionMovie[] collectionMovies = collectionAppends.Parts.ToList()
        ...
        await _context.CollectionMovie.UpsertRange(collectionMovies)
    ...
    private async Task DispatchJobs()
    {
        ...
        foreach (TmdbMovie movie in collectionAppends.Parts)
        {
            TmdbMovieClient tmdbMovieClient = new(movie.Id);
            TmdbMovieAppends? movieAppends = await tmdbMovieClient.WithAllAppends();
    ...
    {
        Translation[] translations = collectionAppends.Translations.Translations.ToList()
