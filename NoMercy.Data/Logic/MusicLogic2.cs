...
public partial class MusicLogic2(Library library, string inputFile) : IAsyncDisposable
{
    ...
    {
        Album? album = MediaContext.Albums
        ...
        bool shouldRemove = album.AlbumTrack.All(t => t.Track.Filename is null && t.Track.Duration is null);
        if (!shouldRemove) return;

        Guid[] trackIds = album.AlbumTrack.Select(t => t.TrackId).ToArray();
        foreach (Guid trackId in trackIds)
        {
            Track? track = await MediaContext.Tracks.FindAsync(trackId);
    ...
    private bool FindMatchByFingerprint()
    {
        ...
        IEnumerable<AcoustIdFingerprintRecording> recordings = FingerPrint.Results
        ...
        AcoustIdFingerprintReleaseGroups[] releases = recordings
    ...
    private async Task Init()
    {
        ...
        Match reg = Regex.Match(FilePath, "(([\\\\\\/].+)([\\\\\\/].+))([\\\\\\/].+)");
    ...
    {
        string artistName = artist.RemoveDiacritics().RemoveNonAlphaNumericCharacters();
    ...
    private async Task StoreRelease(MusicBrainzReleaseAppends musicBrainzRelease)
    {
        ...
        bool shouldInsert = sanitizedAlbumName != "" && SanitizedAlbumFolderName.Contains(sanitizedAlbumName);
        ...
        Album albumInsert = new()
        {
            ...
            Year = musicBrainzRelease.DateTime?.ParseYear() ??
                   musicBrainzRelease.MusicBrainzReleaseGroup.FirstReleaseDate.ParseYear(),
    ...
    private async Task StoreArtist(MusicBrainzArtist? artist)
    {
        ...
        string artistFolder = MakeArtistFolder(artist.Name);

        artistInsert.Folder = artistFolder;
        artistInsert.HostFolder =
            Path.Join(Library.FolderLibraries.FirstOrDefault()?.Folder.Path, artistFolder).PathName();
        ...
        try
        {
            ...
            FanArtImagesJob fanartImagesJob = new(artist);
            JobDispatcher.Dispatch(fanartImagesJob, "image", 2);
    ...
    private async Task<MusicBrainzTrack?> StoreTrack(MusicBrainzReleaseAppends musicBrainzRelease,
        MusicBrainzTrack track, MusicBrainzMedia musicBrainzMedia)
    {
        await using MediaContext mediaContext = new();
        ...
        bool shouldInsert = sanitizedTrackName != "" && SanitizedFileName.Contains(sanitizedTrackName);
        ...
        string? file = FileMatch(musicBrainzRelease, musicBrainzMedia, trackInsert);
        ...
        int numberOfAlbums, int trackNumber, int padding = 2)
    {
        ...
        {
            string fileName = Path.GetFileName(f).RemoveDiacritics().RemoveNonAlphaNumericCharacters().ToLower();
    ...
    private static async Task LinkArtistToAlbum(MusicBrainzArtist musicBrainzArtistMusicBrainzArtist,
    ...
    private static async Task LinkArtistToTrack(MusicBrainzArtist musicBrainzArtistMusicBrainzArtist,
